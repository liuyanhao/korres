/*
 * Copyright 2018-2020 yanhaoit.top. All rights reserved.
 * Support: http://www.yanhaoit.top
 * License: http://www.yanhaoit.top/license
 * 
 * JavaScript - Common
 * Version: 3.0
 */
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: korres.base + "/admin/file/browser.jhtml",
				uploadUrl: korres.base + "/admin/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = korres.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = korres.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
