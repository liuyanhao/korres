...
[@seo type = "productContent"]
	<title>[#if product.seoTitle??]${product.seoTitle}[#elseif seo.title??][@seo.title?interpret /][/#if][#if systemShowPowered] - Powered By KORRES[/#if]</title>
			...
			$.ajax({
				...
				success: function(message) {
					...
					if(cartQuantity == null || cartQuantity == ""){
						...
						$("#cartQuantity").text(parseInt(cartQuantity) + 1);
					}
				}
			});
		} else {
